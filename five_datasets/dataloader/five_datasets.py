...
from .augmentations import CIFAR10Policy, SVHNPolicy
class combine_x_y(Dataset):
    def __init__(self, x, y):
        super(combine_x_y, self).__init__()
        self.x = x
        self.y = y
...
def get_five_loaders(data, task, batch_size_train, batch_size_test):
    dataset_train = combine_x_y(data[task]['train']['x'], data[task]['train']['y'])
    dataset_test = combine_x_y(data[task]['test']['x'], data[task]['test']['y'])
...
def get(seed=1,fixed_order=False,pc_valid=0.05):
    ...
    size=[3,32,32]
    cur_wd = os.getcwd()
    ppath = "five_datasets/dataloader"
    os.chdir(ppath)
    idata=np.arange(5)
    ...
    if not os.path.isdir('./data/Five_data/binary_mixture_5_Data/'):
        ...
        for n,idx in enumerate(idata):
            if idx==0:
                ...
                dat['train']=datasets.CIFAR10('./data/Five_data/',train=True,download=True,transform=train_transforms)
                dat['test']=datasets.CIFAR10('./data/Five_data/',train=False,download=True,transform=common_transforms)
                data[n]={}
                data[n]['name']='cifar10'
                data[n]['ncla']=10
                for s in ['train','test']:
                ...
                dat['train']=datasets.SVHN('./data/Five_data/',split='train',download=True,transform=train_transforms)
                dat['test']=datasets.SVHN('./data/Five_data/',split='test',download=True,transform=common_transforms)
                data[n] = {}
                data[n]['name']='svhn'
                data[n]['ncla']=10
                for s in ['train','test']:
        ...
        for n,idx in enumerate(idata):
            ...
            for s in ['train','test']:
                data[n][s]={'x':[],'y':[]}
                data[n][s]['x'] = torch.load(os.path.join(os.path.expanduser('/media/local/workspace/CL_workspace/Transformer_CL_old/data/Five_data/binary_mixture_5_Data'),'data'+str(idx)+s+'x.bin'))
                data[n][s]['y'] = torch.load(os.path.join(os.path.expanduser('/media/local/workspace/CL_workspace/Transformer_CL_old/data/Five_data/binary_mixture_5_Data'),'data'+str(idx)+s+'y.bin'))
